...
export default function ToDoCard({ item, index }) {
	const icon = {
		...
		discussion_topic: (
			<RiQuestionAnswerFill className="text-2xl text-pink-500" />
		),
	};
	// console.log(item);
	return (
		<div
			key={index}
			className={`mb-4 flex w-full items-center justify-between rounded-lg p-4 shadow-md ${item.marked_complete ? "bg-gray-200 line-through" : "bg-pink-100"}`}
		>
			<div>
				{icon[item.type]}
				<a
