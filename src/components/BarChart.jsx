...
const BarChart = ({ data, value, label, title }) => {
	...
	const init = () => {
		...
		svg = container
			...
			.attr(
				"viewBox",
				`${settings.viewBox.x} ${settings.viewBox.y} ${settings.viewBox.width} ${settings.viewBox.height}`,
			)
	...
	const paintData = () => {
		...
		const stackedData = stack(
			dataset.map((d) => ({ ...d.value, label: d.label })),
		);
		...
		svg.append("g")
			...
			.attr(
				"y",
				settings.labels.y +
					settings.labels.height -
					settings.labels.baseline,
			)
